...
LANDING_START = "# LANDING:START (generated - do not edit by hand)"
...
def main() -> int:
    ...
    # ---- landing-page data --------------------------------------------------
    # The homepage stat tiles and announcement bar read from here, so a number
    # on the landing page can never drift from data/*.yml. Written into
    # mkdocs.yml's `extra:` between markers, which is how a Jinja template can
    # reach it as config.extra.landing.
    ann = load("announcement.yml") or {}

    def decision_short(d):
        return (d["title"].split("?")[0]
                .replace("What risk tier is this AI system", "Risk tier")
                .replace("Are we the provider or the deployer of this system",
                         "Provider or deployer")
                .strip())

    landing = {
        "announcement": {
            "show": bool(ann.get("show", False)),
            "text": " ".join(str(ann.get("text", "")).split()),
            "link_text": " ".join(str(ann.get("link_text", "")).split()),
            "link_url": str(ann.get("link_url", "")),
        },
        "reviewed": max(
            (str(x["reviewed"]) for x in templates if x.get("reviewed")),
            default=""),
        "tiles": [
            {"value": str(len(templates)),
             "label": "Board-ready templates",
             "note": f"{n_ready} fully drafted",
             "tone": "good"},
            {"value": str(len(questions)),
             "label": f"Assessment questions \u00b7 {len(roles)} roles",
             "note": "Evidence guidance on every one"},
            {"value": str(len(decisions)),
             "label": "Guided decisions to a verdict",
             "note": " \u00b7 ".join(decision_short(d) for d in decisions)},
            {"value": "0",
             "label": "Accounts, uploads, trackers",
             "note": "Everything stays in your browser"},
        ],
    }
    _body = yaml.safe_dump({"landing": landing}, sort_keys=False,
                           allow_unicode=True, default_flow_style=False,
                           width=100).rstrip()
    landing_yaml = "\n".join(
        ["  " + LANDING_START]
        + ["  " + ln if ln.strip() else ln for ln in _body.split("\n")]
        + ["  " + LANDING_END])
    ...
    if LANDING_START in text and LANDING_END in text:
        text = re.sub(
            re.escape("  " + LANDING_START) + r".*?" + re.escape(LANDING_END),
            lambda _m: landing_yaml,
            text, flags=re.S,
        )
